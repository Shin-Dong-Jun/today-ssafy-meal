...
서비스가 아니며, 저장소의 초기 식단은 **실제 식단이 아닌 화면 확인용 샘플**입니다.
...
백엔드 서버, 데이터베이스, 인증, 웹사이트 내부 AI API는 사용하지 않습니다.
...
1. 해당 주의 A4 식단표 사진을 Codex 대화에 첨부합니다.
2. Codex에 사진에서 **확실하게 읽히는 내용만** `src/data/meals.ts`에 반영해
   달라고 요청합니다.
3. `weekStart`를 해당 주 월요일의 `YYYY-MM-DD`로 바꿉니다.
4. 월요일부터 금요일까지 각 `date`, `dayOfWeek`, `menuItems`를 갱신합니다.
...
사진 분석은 배포된 웹사이트가 아니라 사용자가 사진을 첨부한 Codex 작업에서만
이루어집니다.
...
초기 데이터는 반드시 `isSample: true` 상태를 유지합니다. 실제 식단표를 반영하지
않은 채 이 값을 `false`로 바꾸면 안 됩니다.
