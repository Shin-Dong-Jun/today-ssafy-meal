...
- 변경 후 `npm test`, `npm run lint`, `npm run typecheck`, `npm run build`를 실행한다.
- 일반 작업은 사용자의 허락 없이 커밋하거나 원격 저장소에 Push하지 않는다.
- 단, 사용자가 제공한 식단표 사진을 반영하는 작업은 검증이 성공하면 별도 확인 없이 작업 브랜치 생성, 커밋, Push, PR 생성 및 `main` 병합까지 완료한다.
- 사용자가 식단 갱신 작업의 중단이나 보류를 요청하면 Git 작업을 진행하지 않는다.
